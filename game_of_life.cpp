...
void start();
void read_input_file(string file_name, coordinate sec[], int& last, int& generation);
string get_user_input();
void display(int grid[GRID_X][GRID_Y]);
void display_coordinates(coordinate coordinates[], int count);
void find_dead_neihbor(int grid[GRID_X][GRID_Y], coordinate target, coordinate nei[], int & count);
void insert_dead(int grid[GRID_X][GRID_Y], int x, int y, coordinate nei[], int & count);
...
vector<coordinate> apply_rule_birth(int grid[GRID_X][GRID_Y], coordinate nei[], int nei_size);
vector<coordinate> apply_rule_survival(int grid[GRID_X][GRID_Y], coordinate sec[], int last);
vector<coordinate> apply_rule_loneliness(int grid[GRID_X][GRID_Y], coordinate nei[], int nei_size);
vector<coordinate> apply_rule_overcrowding(int grid[GRID_X][GRID_Y], coordinate nei[], int nei_size);
...
void write_output_file(coordinate sec[], int& last, int & generation);
...
void start() {
    ...
    read_input_file(file_name, sec, last, generation);
    ...
    write_output_file(sec,last,generation);
...
void find_dead_neihbor(int grid[GRID_X][GRID_Y], coordinate target, coordinate nei[], int & count) {
    insert_dead(grid, target.x - 1, target.y - 1, nei, count);
    insert_dead(grid, target.x, target.y - 1, nei, count);
    insert_dead(grid, target.x + 1, target.y - 1, nei, count);
    insert_dead(grid, target.x - 1, target.y, nei, count);
    insert_dead(grid, target.x + 1, target.y, nei, count);
    insert_dead(grid, target.x - 1, target.y + 1, nei, count);
    insert_dead(grid, target.x, target.y + 1, nei, count);
    insert_dead(grid, target.x + 1, target.y + 1, nei, count);
...
void insert_dead(int grid[GRID_X][GRID_Y], int x, int y, coordinate nei[], int & count) {
    if (grid[x][y] == 0 && x >= 0 && y >= 0 && is_exist(nei, count, x, y) == 1) {
        coordinate die;
        die.x = x;
        die.y = y;
        nei[count++] = die;
...
vector<coordinate> apply_rule_birth(int grid[GRID_X][GRID_Y], coordinate nei[], int nei_size)
{
    ...
    {
        int dead_neighbors = count_neighbors(grid, nei[i]);
        if (dead_neighbors == 3)
        {
            birth_result.push_back(nei[i]);
